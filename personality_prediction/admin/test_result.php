...
<input type="text" id="myInput" class="form-control" placeholder="Filter Result.." autofocus="">
       ...
       <div class="table-responsive">
       	<table class="table table-striped table-hover table-bordered">
       		...
       		<tbody id="myTable">
       			<?php
       			$numdata = mysqli_query($con,"SELECT * FROM `tbl_test` WHERE `uid` = '".$id."' ");
       			$numdata1 = mysqli_num_rows($numdata);
       			if($numdata1==5)
       			{
       			$rdata = mysqli_query($con,"SELECT * FROM `tbl_test` WHERE `uid` = '".$id."' ");
       			$counter = 1;
       			$total = 0;
       			...
       			<tr>
       				...
       				<td>
       				    ...
       				    ?>
                                  <p><b>Category: </b><?php echo $row1['category'] ?></p>
       				    <p><b>Que: </b><?php echo $row1['que'] ?></p>
       				    <ol type="A">
       		...
       		<?php $counter++;}}else{ echo '<script>alert("Finish test first...");window.location.href="result";</script>';} ?>
       		</tbody>
       	</table>
       </div>
		<!-- result table end -->
      ...
      <div class="well">
		<p><b>Candidate Name : </b><?php 
            $un = mysqli_query($con,"SELECT * FROM `tbl_users` WHERE `uid` = $id ");
            $un1 = mysqli_fetch_array($un);
            echo $un1['uname'];
             ?></p>
